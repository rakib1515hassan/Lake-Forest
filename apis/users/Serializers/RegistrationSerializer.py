...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if len(attrs["password"]) < 8 or not any(
            char.isalpha() for char in attrs["password"]
        ):
            raise serializers.ValidationError(
    ...
    def create(self, validated_data):
        ...
        if role == "member":
            ...
            raise serializers.ValidationError(
                {"Please select a role either `member` or `mentor`."}
            )
        ...
        html_content = render_to_string(
            "mail/otp_mail.html", {"user": user, "code": otp_obj.otp}
        )
